...
| Update Delivery Status      | `PATCH /rider/orders/:orderId/status` | Update the delivery status of an order                  | `{ orderStatus: "pickedUp"\|"onTheWay"\|"delivered"\|"undeliverable" }`                                                                                                                                                                                                                                                                                                                                                               | `{ message: String, data: { order } }`                                                                                                                     | `"Delivery status updated"`                    | `"Order not found"` (404), `"Invalid status transition"` (400), `"Session Expired"` (401) |
...
```json
{
  ...
  "orderStatus": "pending | accepted | preparing | ready | pickedUp | onTheWay | undeliverable | delivered | cancelled | failed | rejected",
  "rating": "Number (1-5)",
  "billDetails": {
    "totalAmount": "Number",
    "platformFee": "Number",
